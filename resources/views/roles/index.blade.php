...
@section('content')
    ...
    @can('role_create')
    <p>
        <a href="{{ route('roles.create') }}" class="btn btn-success">@lang('panel.add_new')</a>
    </p>
    @endcan
                    ...
                    @if (count($roles) > 0)
                        @foreach ($roles as $role)
                            <tr data-entry-id="{{ $role->id }}">
                                ...
                                <td>
                                    @can('role_view')
                                    <a href="{{ route('roles.show',[$role->id]) }}" class="btn btn-xs btn-primary">@lang('panel.view')</a>
                                    @endcan
                                    @can('role_edit')
                                    <a href="{{ route('roles.edit',[$role->id]) }}" class="btn btn-xs btn-info">@lang('panel.edit')</a>
                                    @endcan
                                    @can('role_delete')
                                    {!! Form::open(array(
